...
const update = () => {
    ...
    form.put(route("other-income.update", props.otherIncome?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close");
            form.reset();
...
watchEffect(() => {
    if (props.show) {
        var date = new Date(props.otherIncome?.date);
        date.setHours(date.getHours() + 8);

        form.errors = {};
        form.amount = props.otherIncome?.amount;
        form.description = props.otherIncome?.description;
