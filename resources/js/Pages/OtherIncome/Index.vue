...
import Create from "@/Pages/Transaction/OtherIncome/Create.vue";
import Edit from "@/Pages/Transaction/OtherIncome/Edit.vue";
import Delete from "@/Pages/Transaction/OtherIncome/Delete.vue";
import DeleteBulk from "@/Pages/Transaction/OtherIncome/DeleteBulk.vue";
...
watch(
    ...
    debounce(() => {
        ...
        router.get(route("other-income.index"), params, {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        });
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.otherIncomes?.data.forEach((otherIncome) => {
            data.selectedId.push(otherIncome.id);
        });
    }
};
const select = () => {
    if (props.otherIncomes?.data.length == data.selectedId.length) {
    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-4">
            ...
            >
                ...
                >
                    <Pagination :links="props.otherIncomes" :filters="data.params" />
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
